...
**概率语义分型（v0.7.0 新增——全体系数值论断的唯一语义规则，适用于所有产物，非仅风险条目）**：
       ...
       - **C 型·主观判断预测**（基准率+个案调整，如"本标的破发概率 15%"）：数值**只允许出现在 S5 预测记录及报告「预测与验证计划」区块**，正文叙述一律用三档速记。S5 内强制：base_rate_ref（锚定的 B 型基准）+ 偏离理由（一句话）+ 点估计（允许附区间表达不确定度）。**指针格式（v0.7.2）**：产物引用预测只用"预测 ID + 三档"，禁止抄录点估计（场景 9 实测失败模式）
