...
## G8: 模板合规（Template Compliance，v0.4.2 新增）

**规则**：输出报告必须包含官方模板（references/templates/）定义的**固定区块**：
       - 执行摘要（节点评级 / TOP 风险 / 关键数据）
       - 关键论断时效清单（pricing Step 4.5 产出）
       - 制品新鲜度
       HTML 的视觉结构与样式可自定义，固定区块不可缺、不可改名替换（实测反例：以"方法论声明"替代时效清单）。
**不通过处理**：标注缺失区块，要求补全后重新输出。
...
**回灌检查（v0.4.3 新增）**：D4/[STALE_DATA] 标注不是终点。完成时效判定清单后，必须逐字扫描结论位——执行摘要、TOP 风险、关键数据、各节点 key_finding、overall_rating 依据——凡出现被 D4 标注的论断数值，**删除或改写**该处表述。实测反例：v0.4.2 某次推演清单正确标注"2024 年市场数据超期，不入执行摘要/TOP 风险"，而同一份报告的 TOP 1 风险与综合结论仍使用该数据——判而不行，视同门禁未通过。
