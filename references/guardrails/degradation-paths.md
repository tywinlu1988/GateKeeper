...
**语义边界（v0.4.2 新增）**：D4 **仅**用于时效不足。
- 多来源数值冲突 → 走 D3（保留矛盾双方），不得借 D4 掩盖矛盾
- 单一来源的极端异常值 → 不是降级情形：在 rationale 中说明剔除理由即可；若有条件应补查一次确认该值真实存在（实测反例：把编错的 PE 206x 标为 D4"异常值隔离"，而真实值 ~149x 从未被复核发现）
