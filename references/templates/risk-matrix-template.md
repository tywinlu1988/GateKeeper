# 风险矩阵输出模板（DEPRECATED，v0.7.0 起）

> ⚠️ 已弃用：v0.5.0 起风险矩阵仅为可选底稿附录，v0.7.0 起正式弃用——主交付物为 final-report.html（三产物 + 预测与验证计划）。本文件保留仅供历史报告对照，禁止用于新推演。
...
*报告由 Gatekeeper v0.8.6 生成 · 所有数据均内联摘要，可独立分发*
