...
description: >
  ...
  路由到 industry 节点时需要执行该节点推演时。不独立触发——无分析
...
按 `references/guardrails/quality-gates.md` **全部**门禁逐项检查（当前为 G1-G8，含 G1.5 信号完备、G1.6 交叉信号一致性、G1.7 异议角色加权、G7 时效合规、G8 模板合规）。门禁清单的唯一事实源是 quality-gates.md——此处不复制完整清单，避免漂移。
...
如需自包含 HTML 报告，**以 `references/templates/risk-matrix-template.html` 为底本填充**（内联 CSS，可独立分发，满足非协商条款 N8）——禁止从零自行设计 HTML/CSS；输出必须保留模板标记注释（G8 验证）。HTML 生成必须遵循 `references/templates/html-assembly.md` 组装协议（Write 分块 + cat 拼接；禁止 Bash heredoc / 依赖 python 渲染）。
